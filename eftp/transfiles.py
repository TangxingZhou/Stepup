...
class TransFile:
    def __init__(self, local_path, remote_path, host, user, password, files=[], port=FTP_PORT, up_or_down='UP'):
        self.port = port
        self.files = files
                    ...
                    try:
                        ...
                        for myfile in self.files:
                            ...
                            with open(filename, 'rb') as mfile:
                                try:
                                    remotefile = myfile.replace(self.localSymbol, self.remoteSymbol)
                                    mftp.storbinary('STOR ' + remotefile, mfile, bufsize)
                                except Exception as e:
                                    print('Error: Failed to upload "', filename, '" to "', self.__host, '" into "',
                                          self.remote_path, '"\nInfo: ', e)
                            print('"', filename, '" uploaded successfuly!')
                        mftp.set_debuglevel(0)
                    except Exception as e:
                        print('Error: Failed to get into the remote path "', self.remote_path, '"\nInfo: ', e)
                    finally:
                        mftp.quit()
                        print('All files are uploaded successfully!')
                except Exception as e:
                    print('Error: Failed to login "', self.__host, '" as "', self.__user, '"\nInfo: ', e)
            except Exception as e:
                print('Error: Failed to connect to "', self.__host, '" via FTP\nInfo: ', e)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        bufsize = 1024
                        for myfile in self.files:
                            localfile = myfile.replace(self.remoteSymbol, self.localSymbol)
                            if sys.platform == 'win32':
                            ...
                            with open(filename, 'wb') as mfile:
                                try:
                                    mftp.retrbinary('RETR ' + myfile, mfile.write, bufsize)
                                except Exception as e:
                                    print('Error: Failed to download "', filename, '" from "', self.__host, '" into "',
                                          self.local_path, '"\nInfo: ', e)
        ...
        if self.port == FTP_PORT:
            ...
            if os.path.isdir(self.local_path):
                try:
                    ...
                    print('Can not access "', self.local_path, '" at local\nInfo: ', e)
            else:
                return 1
            try:
                mftp.connect(self.__host, FTP_PORT)
                            ...
                            if upOrDown == 'DOWN':
                                ...
                                with open(filename, 'wb') as mfile:
                                    try:
                                        ...
                                        print('Error: Failed to download "', filename, '" from "', self.__host,
                                              '" into "', self.local_path, '"\nInfo: ', e)
                                print('"', filename, '" downloaded successfuly!')
                            elif upOrDown == 'UP':
                                with open(filename, 'rb') as mfile:
                                    try:
                                        mftp.storbinary('STOR ' + myfile, mfile, bufsize)
                                    except Exception as e:
                                        print('Error: Failed to upload "', filename, '" to "', self.__host, '" into "',
                                              self.remote_path, '"\nInfo: ', e)
                                print('"', filename, '" uploaded successfuly!')
    ...
    def dispatchFiles(self, mftp, up_or_down, bufsize=1024):
        print('"', threading.current_thread().name, '" is calling the function of dispatchFiles')  # 打印当前线程名
        lock.acquire()  # 获取同步锁
        if len(self.files)>0:
            ...
            lock.release()  # 释放同步锁
            print('"', threading.current_thread().name,'": No files to be transfered\n')
            return 0
        if file_yes_or_no == True:
            filename = self.local_path + self.localSymbol + myfile  # 获取文件全路径
            if up_or_down == 'DOWN':
                if self.localSymbol != self.remoteSymbol:
                    filename = filename.replace(self.remoteSymbol, self.localSymbol)
                with open(filename, 'wb') as mfile:  # 以二进制写的方式打开文件
                    try:
                        mftp.retrbinary('RETR ' + myfile, mfile.write, bufsize)  # 下载并写入本地文件
                    except Exception as e:
                        print('Error: Failed to download "', filename, '" from "', self.__host, '" into "',
                              self.local_path, '"\nInfo: ', e)
                print(threading.current_thread().name,': "', filename, '" downloaded successfuly!')
            elif up_or_down == 'UP':
                with open(filename, 'rb') as mfile:  # 以二进制读的方式打开文件
                    try:
                        if self.localSymbol != self.remoteSymbol:
                            myfile = myfile.replace(self.localSymbol, self.remoteSymbol)
                        mftp.storbinary('STOR ' + myfile, mfile, bufsize)  # 上传文件到远程服务器
                    except Exception as e:
                        print('Error: Failed to upload "', filename, '" to "', self.__host, '" into "',
                              self.remote_path, '"\nInfo: ', e)
                print(threading.current_thread().name,': "', filename, '" uploaded successfuly!')
            else:
    ...
    def startThreads(self, up_or_down, nbrofThreads=0):
        for i in range(nbrofThreads):
            ...
            t = threading.Thread(target=self.dispatchFiles, args=tuple([mftp,up_or_down]))
            t.start()
                ...
                if os.path.isdir(myfile):  # 出栈元素表示本地路径
                    ...
                    if newlist == []:  # 空目录
                        ...
                        for i in range(len(newlist)):
                            newlist[i] = myfile + self.localSymbol + newlist[i]  # 非空目录则将路径名加到其内容列表的各元素前
                    ...
                    if lastindex == -1:  # 文件在本地工作目录下
                        ...
                        self.mkdir(self.mftp, 'REMOTE', newdir)  # 文件在本地工作目录的子目录下则先在远程创建相应的子目录
                    filename = self.local_path + self.localSymbol + myfile  # 获取本地文件全路径名
                    with open(filename, 'rb') as mfile:  # 以二进制读的方式打开文件
                        try:
                            if self.localSymbol != self.remoteSymbol:
                                myfile = myfile.replace(self.localSymbol, self.remoteSymbol)
                            self.mftp.storbinary('STOR ' + myfile, mfile, bufsize)  # 上传文件到远程服务器
                        except Exception as e:
                            print('Error: Failed to upload "', filename, '" to "', self.__host, '" into "',
                                  self.remote_path + self.remoteSymbol + newdir, '"\nInfo: ', e)
                    print('"', filename, '" uploaded successfuly!')
                else:
                    print('Warning: "', self.local_path + self.localSymbol + myfile, '" does not exist')
            elif up_or_down == 'DOWN':
                try:
                    remotelist = self.mftp.nlst(myfile)  # 列出待检内容
                    if remotelist == []:
                        lastindex = myfile.rfind(self.remoteSymbol)  # 查找路径分界符的最后一个索引
                        if lastindex == -1:
                            nlstpath = ''  # 待检内容在服务器工作目录下
                        else:
                            nlstpath = myfile[:lastindex]  # 待检内容在服务器工作目录的子目录下
                        try:
                            parentlist = self.mftp.nlst(nlstpath)  # 列出待检内容所在目录下的内容
                            if myfile in parentlist:
                                self.mkdir(self.mftp, 'LOCAL', myfile)  # 待检内容在其所在的目录下，为空目录，在本地创建相应的空目录
                            else:
                                print('Warning: "', self.remote_path + self.remoteSymbol + tocheck,
                                      '" does not exist')  # 待检内容不存
                        except Exception as e:
                            print('Error: Failed to list "', self.remote_path + self.remoteSymbol + nlstpath,
                                  '"\nInfo: ', e)
                    else:
                        if [myfile] != remotelist:  # 待检内容为路径
                            try:
                                ...
                                print('Error: Failed to list "', self.remote_path + self.remoteSymbol + myfile,
                                      '"\nInfo: ', e)
                        else:
                            lastindex = myfile.rfind(self.remoteSymbol)  # 查找文件的最后一个路径分界符索引
                            if lastindex == -1:
                                newdir = ''  # 文件在服务器工作目录下
                            else:
                                newdir = myfile[:lastindex]
                                self.mkdir(self.mftp, 'LOCAL', newdir)  # 文件不在服务器工作目录下则先在本地创建对应的子路径
                            filename = self.local_path + self.localSymbol + myfile.replace(self.remoteSymbol,
                                                                                           self.localSymbol)  # 本地文件的全路径
                            with open(filename, 'wb') as mfile:  # 以二进制写的方式打开文件
                                try:
                                    self.mftp.retrbinary('RETR ' + myfile, mfile.write, bufsize)  # 下载并写入本地文件
                                except Exception as e:
                                    print('Error: Failed to download "', filename, '" from "', self.__host, '" into "',
                                          self.local_path + self.localSymbol + newdir, '"\nInfo: ', e)
                            print('"', filename, '" downloaded successfuly!')
                except Exception as e:
                    print('Error: Failed to list "', self.remote_path + self.remoteSymbol + myfile, '"\nInfo: ', e)
    ...
    def mkdir(self, mftp, local_or_remote, path=''):
        if local_or_remote == 'LOCAL':
            if self.localSymbol != self.remoteSymbol:
                localpath = path.replace(self.remoteSymbol, self.localSymbol)  # 替换为本地系统分界符
            if os.path.isdir(localpath) == False:
                try:
                    os.makedirs(localpath)  # 路径不存在则创建递归的路径
                except Exception as e:
                    print('Error: Failed to create directory "', localpath, '" at local\nInfo: ', e)
            return True
        elif local_or_remote == 'REMOTE':
            if self.localSymbol != self.remoteSymbol:
                remotepath = path.replace(self.localSymbol, self.remoteSymbol)  # 替换为远程系统分界符
                ...
                try:
                    ...
                    if subdir not in currentlist:
                        try:
                            ...
                            print('Error: Failed to create directory "', subdir, '" under "', self.mftp.pwd(),
                                  '"\nInfo: ', e)
                    try:
                        mftp.cwd(subdir)  # 创建子目录后进入子目录以创建下一层子目录
                    except Exception as e:
                        print('Error: Failed to get into "', self.mftp.pwd() + self.remoteSymbol + subdir, '"\nInfo: ',
                              e)
                except Exception as e:
                    print('Error: Failed to list current directory "', self.mftp.pwd(), '"\nInfo: ', e)
            try:
        ...
        if up_or_down == 'UP':
            if os.path.isdir(tocheck):  # 待检内容为路径
                ...
                if newlist == []:
                    ...
                    for i in range(len(newlist)):
                        newlist[i] = tocheck + self.localSymbol + newlist[i]  # 非空目录则将路径名加到其内容列表的各元素前
                    self.files.extend(newlist)  # 新的元素列表作为新的待检内容添加到类的待检成员列表
                return False
            elif os.path.isfile(tocheck):
                lastindex = tocheck.rfind(self.localSymbol)  # 取出路径名
                if lastindex != -1:
                    self.mkdir(mftp, 'REMOTE', tocheck[:lastindex])  # 文件在本地工作目录的子目录下则先在远程创建相应的子目录
                return True
            else:
                print('Warning: "', self.local_path + self.localSymbol + tocheck, '" does not exist')
                return False
        elif up_or_down == 'DOWN':
            try:
                remotelist = mftp.nlst(tocheck)  # 列出待检内容
                if remotelist == []:
                    lastindex = tocheck.rfind(self.remoteSymbol)  # 查找路径分界符的最后一个索引
                    if lastindex == -1:
                        nlstpath = ''  # 待检内容在服务器工作目录下
                    else:
                    ...
                    try:
                        ...
                        if tocheck in parentlist:
                            self.mkdir(mftp, 'LOCAL', tocheck)  # 待检内容在其所在的目录下，为空目录，在本地创建相应的空目录
                        else:
                            print('Warning: "', self.remote_path + self.remoteSymbol + tocheck,
                    ...
                    if [tocheck] != remotelist:  # 待检内容为路径
                        try:
                            ...
                            print('Error: Failed to list "', self.remote_path + self.remoteSymbol + tocheck,
                                  '"\nInfo: ', e)
                    else:
                        lastindex = tocheck.rfind(self.remoteSymbol)  # 查找待检内容中的最后一个路径分界符索引
                        if lastindex != -1:
                            self.mkdir(mftp, 'LOCAL', tocheck[:lastindex])  # 文件不在服务器工作目录下则先在本地创建对应的子路径
                        return True
            except Exception as e:
                print('Error: Failed to list "', self.remote_path + self.remoteSymbol + tocheck, '"\nInfo: ', e)
        else:
...
def main():
    ...
    mTrans = TransFile('D:\\Codes\\Python', '/home/tangxinz', '135.251.238.34', 'tangxinz', 'zhou19891001', ['googletest-master'], 21)
