...
def xiaoqu_spider(
    ...
    for xq in xiaoqu_list:
        ...
        content = unicode(
            xq.find(
                'div', {
                    'class': 'con'}).renderContents().strip())
        info = re.match(
...
def do_xiaoqu_spider(db_xq, region=u"昌平"):
    ...
    try:
        ...
        return
    d = "d=" + \
        soup.find('div', {'class': 'page-box house-lst-page-box'}).get('page-data')
    exec(d)
...
def chengjiao_spider(
        db_cj,
        url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD"):
    """
    爬取页面链接中的成交记录
    """
    try:
