...
def gen_xiaoqu_insert_command(info_dict):
    ...
    生成小区数据库插入命令
    """
    info_list=[u'小区名称',u'小区代号',u'大区域',u'小区域',u'小区户型',u'建造时间',u'学校',u'地铁',u'均价',u'成交',u'二手房',u'租房']
    t=[]
    for il in info_list:
        if il in info_dict:
...
def gen_chengjiao_insert_command(info_dict):
    ...
    生成成交记录数据库插入命令
    """
    info_list=[u'链接',u'小区名称',u'户型',u'面积',u'朝向',u'楼层',u'年份',u'装修',u'签约时间',u'签约单价',u'签约总价',u'签约方',u'房型',u'交通学校等']
    t=[]
    for il in info_list:
        if il in info_dict:
            t.append(info_dict[il])
...
def xiaoqu_spider(db_xq,url_page=u"http://bj.lianjia.com/xiaoqu/"):
    ...
    """
    try:
        req = request.Request(url_page,headers=hds[random.randint(0,len(hds)-1)])
    ...
    for xq in xiaoqu_list:
        ...
        content=xq.find('div',{'class':'con'}).renderContents().strip().decode()
        ...
        content_price=xq.find('div',{'class':'price'}).find('span',{'class':'num'}).renderContents().strip().decode()
        price=re.findall('\d+(?=<)',content_price)
        if price!=[]:
            info_dict.update({u'均价':re.findall('\d+(?=<)',content_price)[0]})
        content_ershou=xq.find('div',{'class':'square'}).find('span',{'class':'num'}).text
        ...
        content_zufang=xq.find('a',{'class':'laisuzhou','href':re.compile('.*\/zf\/')}).text
...
def do_xiaoqu_spider(db_xq,region='changping'):
    ...
    for i in range(total_pages):
        if i==0:
            ...
            url_page='http://bj.lianjia.com/xiaoqu/%s/pg%d/' % (region,i+1)
...
def chengjiao_spider(db_cj,url_page=u"http://bj.lianjia.com/chengjiao/"):
    ...
    for cj in cj_list:
        ...
        content=cj.find('div',{'class':'houseInfo'}).renderContents().strip().decode()
        ...
        content=cj.find('div',{'class':'positionInfo'}).renderContents().strip().decode()
        info=re.findall('(?<=span>).*(?=<)',content)
        if len(info)>0:
            if len(info[0])>0:
                houseInfo=info.split()
...
def xiaoqu_chengjiao_spider(db_cj,xq_code='c1111027378998'):
    ...
    try:
        req = request.Request(url,headers=hds[random.randint(0,len(hds)-1)])
        source_code = request.urlopen(req,timeout=10).read()
        plain_text=source_code.decode()
        soup = BeautifulSoup(plain_text)
    #except (urllib.HTTPError, urllib.URLError) as e:
    ...
    for i in range(total_pages):
        if i==0:
            ...
            url_page='http://bj.lianjia.com/chengjiao/pg%d' %(i+1) +xq_code+'/'
