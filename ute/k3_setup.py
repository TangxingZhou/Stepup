...
class K3(SSHService):
    ...
    def __init__(self, ip, user, password):
        super(
            ...
            user,
            password,
            windows_local_home=K3_SETUP_DIR)

    def execute_svn_export(
            self,
            src,
            dest,
            svn_user='ca_ute_test',
            svn_pass='utetest',
            local=True):
    ...
    def svn_export_k3_files(self, local=True):
        ttcn3_dest = 'release{}ttcn3'.format(
            self.local_sep if local is True else self.remote_sep)
        codec_dest = 'release{}codec'.format(
            self.local_sep if local is True else self.remote_sep)
            ...
            if self.sftp_transmit_files((zip_path + '.zip',), 'ul') is True:
                if self.unzip_files(zip_path + '.zip', zip_path) is True:
                    self.execute_command(
                        'sudo mv {0}/opt/{0} /opt'.format(zip_path))
                    self.execute_command(
                        'sudo chown -R ute.ute /opt/{}'.format(zip_path))
                    self.execute_command(
                        'cd /opt/gcc/4.9.4/lib64/;'
                        'rm -f libstdc++.so.6;'
        ...
        if self.zip_files('release', 'k3.zip', local) is True:
            if self.sftp_transmit_files(('k3.zip',), 'ul') is True:
                if self.unzip_files('k3.zip', 'k3') is True:
                    ...
                    self.execute_command(
                        'mv k3/release/ttcn3/* /opt/k3/lib64/k3/plugins/ttcn3/')
                    self.execute_command('sudo chown -R ute.ute /opt/k3')
                    self.rm_dirs_and_zip_files('k3')
    ...
    for tl_id in args.tl_id:
        ...
        if m_all is True or ute.is_dir('/opt/k3') is False:
            cloud = K3(
                m_cloud_ip,
                UTE_USER,
                m_cloud_password) if cloud is None else cloud
            cloud.get_k3_home()
            ute.put_k3_home()
        else:
