...
UTE_TESTLINE_TYPE = {'1234': 'FSMF',
                     ...
                     '4234': 'FSMF',
                     '5234': 'FSIP'}
CONFIGURATION_TO_INIT = ['''"from ute_testline_configuration.schema import UTE_WMP"''',
                         '''""''',
                         '''"tl = UTE_WMP(enb_hw_type='{0}',"''',
                         '''"{0}pdu_ip='10.9.45.94',"'''.format(' ' * 13),
                         '''"{0}pdu_type='ATEN',"'''.format(' ' * 13),
                         '''"{0}enb_pdu_outlet=1,"'''.format(' ' * 13),
                         '''"{0}iphy_pdu_outlet=2)"'''.format(' ' * 13),
                         '''""''']
...
if os.path.exists(UTE_SETUP_DIR) is False:
    ...
    if os.path.isfile(UTE_SETUP_DIR + LOGGING_FILE) is False:
        with open(UTE_SETUP_DIR + LOGGING_FILE, 'w') as fp:
            fp.close()
            format_print('Make file {0} for logging'.format(UTE_SETUP_DIR + LOGGING_FILE))
...
def args_parser(name, description=None, usage='usage: %(prog)s [options] arg1 arg2', version='1.0'):
    ...
    parser.add_argument('-l', '--lip', action='store', required=False, default=LINSEE_HOST_IP[0],
                        choices=LINSEE_HOST_IP, dest='linsee_ip', help='The IP for linsee')
    parser.add_argument('-s', '--lu', action='store', required=True, dest='linsee_user',
                        help='The user name for linsee')
    parser.add_argument('-w', '--lpd', action='store', required=True, dest='linsee_password',
    ...
    parser.add_argument('-i', '--iu', action='store', required=True, dest='isource_user',
                        help='The user name for isource')
    parser.add_argument('-p', '--ipd', action='store', required=True, dest='isource_password',
    ...
    parser.add_argument('-b', '--lib', action='store', required=False, nargs='*', default=[], dest='ute_lib',
                        help='The lib to install or update for UTE')
    parser.add_argument('-e', '--level', action='store', type=int, required=False, choices=range(4), default=1,
                        dest='update_level', help='Update or install robotlte, talibs and utelibs, '
                                                  ...
                                                  '3 is for all.')
    parser.add_argument('--local', action='store_true', required=False, default=False, dest='local',
...
class SSHService(object):
    ...
    @staticmethod
    def svn_co_command(repository_url, dest_repository, user, password, version='HEAD',
        ...
        if version == 'HEAD':
            ...
            svn_command = 'svn co {0} {1} -r {2} --username {3} --password {4} {5}'\
                .format(repository_url, dest_repository, version, user, password, additions)
        return svn_command

    def svn_repository(self, isource_user, isource_password, repository_name, local=False):
        version = UTE_LIBS_VERSION[repository_name]
        if len(self.list_dir(repository_name, local)) is not 0:
            if version == 'HEAD':
                command = 'svn update {0} --non-interactive --trust-server-cert'.format(repository_name)
            else:
                command = 'svn update {0} -r {1} --non-interactive --trust-server-cert'.format(repository_name, version)
            if local is True:
                info = "It's about to update {0} to {1} at local {2}:{3}"\
                    .format(repository_name, version, self.local_system, self.local_path)
            else:
                info = "It's about to update {0} to {1} at {2}@{3}:{4}"\
                    .format(repository_name, version, self.user, self.ip, self.remote_path)
        else:
            command = SSHService.svn_co_command(
                UTE_LIBS_URL[repository_name], repository_name, isource_user, isource_password, version)
            if local is True:
                info = "It's about to checkout {0} to {1} at local {2}"\
                    .format(repository_name, version, self.local_system)
            else:
                info = "It's about to checkout {0} to {1} at {2}@{3}:{4}"\
                    .format(repository_name, version, self.user, self.ip, self.remote_path)
        logger.info(info)
        self.execute_command(command, local, True)

    def svn_list(self, repository_url, local=False):
        std_out, std_err = self.execute_command('svn list {0}'.format(repository_url), local, True)
        if std_err == '':
            result = []
            if local is True:
    ...
    def svn_info(self, repository_dir, processed=False, local=False):
        ...
        if len(self.list_dir(repository_dir, local)) is 0:
            ...
            std_out, std_err = self.execute_command('svn info {}'.format(repository_dir), local, True)
            if std_err == '':
                if local is True:
                    out = std_out.split(self.local_linesep)
                    info_str = std_out
                    ...
                    out = std_out.split(self.remote_linesep)
                    info_str = std_out.replace(self.remote_linesep, self.local_linesep)
                for line in out[:-1]:
                    if ':' in line:
                        elem = line.split(':')
    ...
    def pip_dict(self, lib_kw='', local=False):
        ...
        if std_err == '':
            ...
            std_out.pop()
            for elem in std_out:
                match = re.match(r'({0}[\w\.-]*)\s\((.+)\)'.format(lib_kw), elem)
                if match is not None:
                    lib_name = match.group(1)
                    lib_version = match.group(2)
                    if lib_name in libs:
                        format_print('There is another {0}, change its version from {1} to {2}'
                                     .format(lib_name, libs[lib_name], lib_version))
                        logger.warn('There is another {0}, change its version from {1} to {2}'
                                    .format(lib_name, libs[lib_name], lib_version))
    ...
    def compare_version(version_one, version_two):
        ...
        if len(version_one_list) < len(version_two_list):
            version_one_list = version_one_list + ['0']*(len(version_two_list)-len(version_one_list))
        elif len(version_one_list) > len(version_one_list):
            version_two_list = version_two_list + ['0']*(len(version_one_list)-len(version_two_list))
        else:
            pass
            ...
            if remote_out is None:
                ...
                if local_out[0] in system_support and remote_out[0] in system_support:
                    ...
                    if std_err == '':
                        ...
                        format_print('Fail to get the hostname at {0}@{1}'.format(self.user, self.ip))
                        logger.error('Fail to get the hostname at {0}@{1}'.format(self.user, self.ip))
    ...
    def prepare_to_install(self, wheel_server, local=False):
        if local is True:
            child = subprocess.Popen(['pip2', '-V'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            std_out, std_err = child.communicate()
            error_msg = 'Unable to find pip2 at local {0}\n{1}'.format(self.local_system, std_err)
        else:
            std_out, std_err = self.ssh_run_command('pip2 -V', True)
            error_msg = 'Unable to find pip2 at {0}@{1}\n{2}'.format(self.user, self.ip, std_err)
        if std_err == '':
            match = re.match(r'.*\((.*)\)', std_out)
            if match is None:
                raise AssertionError('Fail to get the version of python for pip2 from "{0}"'.format(std_out))
            else:
                if match.group(1) == 'python 2.7':
                    ...
                    for lib in BASE_LIBS_VERSION:
                        if lib in self.pip_libs:
                            compare_result = SSHService.compare_version(self.pip_libs[lib], BASE_LIBS_VERSION[lib])
                    ...
                    if len(to_install) is not 0:
                        if isinstance(wheel_server, SSHService) is True and \
                                        hasattr(wheel_server, 'get_wheels_for_libs') is True:
                            wheel_server.get_wheels_for_libs(to_install, 'wheelsbase')
                            wheel_server.get_wheels_and_install_libs(to_install, local, 'wheelsbase')
                else:
                    raise AssertionError('Please install Python 2.7 (https://www.python.org/downloads/)')
        else:
            format_print(error_msg)
            logger.error(error_msg)
            raise AssertionError('Please install pip for Python 2.7 (https://pypi.python.org/pypi/pip)')
    ...
    def make_wheels(self, libs, wheel_home=WHEELS_HOME, local=False):
        ...
        for lib in libs:
            std_out, std_err = self.execute_command('pip2 wheel --wheel-dir={0}/{1} {1}'.format(wheel_home, lib), local)
            if std_err == '':
                make_yes.append(lib)
                logger.info(std_out)
            else:
    ...
    def install_wheel_files(self, files_path, local=False, ignore_version=False):
        install_no = []
        install_yes = []
        file_pip = ''
        new_version = ''
            ...
            if file_path.endswith('.whl') is True:
                if self.is_file(file_path, local) is True:
                    if ignore_version is False:
                        file_name = file_path.split(self.local_sep if local is True else self.remote_sep)[-1]
                        new_version = SSHService.get_version_from_wheel_file(file_name)
                        try:
                            file_pip = file_name[:file_name.index('-')].replace('_', '-')
                        except Exception, e:
                            file_pip = ''
                            logger.debug('Fail to get the lib name from{0}\n{1}'.format(file_name, e))
                        if file_pip in self.pip_libs:
                            old_version = self.pip_libs[file_pip]
                        else:
                            old_version = '0.0.0'
                        version_compare = SSHService.compare_version(new_version, old_version)
                        if version_compare is not 1:
                            logger.info('No need to update {0}, for the old version is {1} and the new version is {2} '
                                        'from {3}'.format(file_pip, old_version, new_version, file_name))
                            continue
                    out = self.execute_command('pip2 install {0} --no-deps'.format(file_path), local)
                    if out[1] == '':
                    ...
                    if local is True:
                        logger.error('{0} does not exists at local {1}'.format(file_path, self.local_system))
                    else:
                        logger.error('{0} does not exists at {1}@{2}'.format(file_path, self.user, self.ip))
            else:
                if local is True:
                    logger.error('{0} is not a wheel file at local {1}'.format(file_path, self.local_system))
                else:
                    logger.error('{0} is not a wheel file at {1}@{2}'.format(file_path, self.user, self.ip))
    ...
    def install_libs(self, libs, wheels_home=WHEELS_HOME, local=False):
        for lib in libs:
            if local is True:
                ...
                lib_home = self.remote_path + wheels_home + self.remote_sep + lib + self.remote_sep
                ...
                for wheel in wheels_for_lib:
                    ...
                    if lib_to_wheel in wheel:
                        if lib_wheel == '':
                            ...
                            format_print('More than one wheel for {0} is found under {1}'.format(lib, lib_home))
                            logger.error('More than one wheel for {0} is found under {1}'.format(lib, lib_home))
                            lib_wheel = ''
                            break
                    if wheel_system == 'Any':
                        ...
                        if local is True:
                            if self.local_system == wheel_system:
                                ...
                                logger.info('{0} is for {1}, but the OS is {2} at local'
                                            .format(wheel, wheel_system, self.local_system))
                        else:
                            if self.remote_system == wheel_system:
                                wheels_support.append(wheel)
                            else:
                                wheels_not_support.append(wheel)
                                logger.info('{0} is for {1}, but the OS is {2} at {3}@{4}'
                                            .format(wheel, wheel_system, self.local_system, self.user, self.ip))
                if lib_wheel == '':
                    self.update_no[lib] = ([], wheels_for_lib)
                else:
                    lib_version = self.get_version_from_wheel_file(lib_wheel, lib_to_wheel)
                    if lib_version == '':
                        self.update_no[lib] = ([], wheels_for_lib)
                    else:
                        if lib in self.pip_libs:
                            ...
                            old_version = '0.0.0'
                        version_compare = self.compare_version(lib_version, old_version)
                        if version_compare is 1:
                            ...
                            if len(wheels_no) is 0:
                                ...
                                self.update_no[lib] = (wheels_yes, wheels_no, wheels_not_support)
                        else:
        ...
        if self.is_file(zip_path, local) is True:
            ...
            if local is True:
                try:
                    ...
                    logger.error('Fail to unzip {0} at local {1}\n{2}'.format(zip_path, self.local_system, e))
            else:
    ...
    def get_robotlte(self, isource_user, isource_password, repository_name='robotlte', local=False):
        self.svn_repository(isource_user, isource_password, repository_name, local)
        if self.zip_files(repository_name, repository_name + '.zip') is True:
            self.sftp_transmit_files((repository_name + '.zip',), 'dl')
        else:
            logger.error('Fail to zip {0} to {0}.zip at {1}@{2}'.format(repository_name, self.user, self.ip))
            raise AssertionError('Fail to zip {0} to {0}.zip at {1}@{2}'.format(repository_name, self.user, self.ip))

    def install_robotlte(self, local=False):
        if local is False:
            self.sftp_transmit_files(('robotlte.zip',), 'ul')
        self.robotlte_ready = self.unzip_files('robotlte.zip', 'robotlte', local)
    ...
    def get_wheels_and_install_libs(self, libs, local=False, wheels_home=WHEELS_HOME):
        if local is False:
    ...
    def ute_setup(self, libs, wheel_server, tl_type='FSMF', update_robotlte=True, local=False, verbose=False):
        ute_setup_threads = []
        if local is True:
        ...
        if update_robotlte is True:
            robotlte_thread = threading.Thread(target=self.install_robotlte, args=(local,))
            robotlte_thread.start()
            logger.info('Thread: {0} starts'.format(robotlte_thread.name))
            logger.info('Thread: {0} calls function {1} with arguments {2}'
                        .format(robotlte_thread.name, self.install_robotlte, (local,)))
            ute_setup_threads.append(robotlte_thread)
        if len(libs) is not 0:
            self.prepare_to_install(wheel_server, local)
            libs_thread = threading.Thread(target=self.get_wheels_and_install_libs, args=(libs, local))
            libs_thread.start()
            logger.info('Thread {0} starts'.format(libs_thread.name))
            logger.info('Thread {0} calls function {1} with arguments {2}'
                        .format(libs_thread.name, self.get_wheels_and_install_libs, (libs, local)))
    ...
    def complete(self, libs, tl_type='FSMF', update_robotlte=True, local=False, verbose=False):
        if update_robotlte is True:
            if self.robotlte_ready is True:
                if local is False:
                    self.ssh_run_command('cd {0}robotlte/;python setup.py develop -U'.format(self.remote_path))
                    self.ssh_run_command('sed -i "s/Pdu Outlet Reset/# Pdu Outlet Reset/g" '
                                         '{0}robotlte/resources/DevWro1/wts_related.robot'.format(self.remote_path))
                    if self.tl_name is not None and self.tl_name not in UTE_HOST_IP:
                        host_name = self.tl_name.replace('-', '_')
                        host_config_dir = '{0}robotlte/config/Shanghai/{1}/'.format(self.remote_path, host_name)
                        site_dir_init = '{0}robotlte/config/Shanghai/__init__.py'.format(self.remote_path)
                        ...
                        if self.is_dir(host_config_dir) is False:
                            self.ssh_run_command('mkdir -p {0}'.format(host_config_dir))
                        else:
                            logger.info('{0} exists at {1}@{2}'.format(host_config_dir, self.user, self.ip))
                        if self.is_file(site_dir_init) is False:
                            self.ssh_run_command('touch {0}'.format(site_dir_init))
                        else:
                            logger.info('{0} exists at {1}@{2}'.format(site_dir_init, self.user, self.ip))
                        if self.is_file(host_dir_init) is False:
                            self.ssh_run_command('touch {0}'.format(host_dir_init))
                            for index, line in enumerate(CONFIGURATION_TO_INIT):
                                if index is 2:
                                    tl_type_config = line.format(tl_type)
                                    self.ssh_run_command('''echo {0}>>{1}'''.format(tl_type_config, host_dir_init))
                                else:
                                    self.ssh_run_command('''echo {0}>>{1}'''.format(line, host_dir_init))
                        else:
                            logger.info('{0} exists at {1}@{2}'.format(host_dir_init, self.user, self.ip))
                    else:
                        format_print('Hostname of {0}@{1} is Unknown'.format(self.user, self.ip))
                        logger.error('Hostname of {0}@{1} is Unknown'.format(self.user, self.ip))
                ...
                if info == '':
                    ...
                    format_print('The robotlte:{0}{1}'.format(self.local_linesep, info))
                    logger.info('The robotlte:{0}{1}'.format(self.local_linesep, info))
            else:
        ...
        format_print(msg)
        logger.info(msg)
        if robotlte_report != 'Fail to install robotlte' and len(self.update_no) is 0:
            tls_update_result[self.tl_name] = 'PASS'
        if local is True:
            format_print('Complete to setup UTE at local {0}'.format(self.local_system))
            logger.info('Complete to setup UTE at local'.format(self.local_system))
        else:
            format_print('Complete to setup UTE at {0}@{1}'.format(self.user, self.ip))
            logger.info('Complete to setup UTE at {0}@{1}'.format(self.user, self.ip))
        tls_update_report[self.tl_name] = '{1:<25}{2:<25}{3}{0}{4:-^60}{0}{5}{0}{6:-^60}{0}{7}{0}{8:=^60}{0}'\
            .format(os.linesep, self.tl_name, 'IP: ' + ('127.0.0.1' if local is True else self.ip), '{:^10}',
                    'Robotlte', robotlte_report, 'Ta & Ute', ta_ute_report, '')
        for zip_file in ['robotlte.zip', WHEELS_HOME + '.zip', 'wheelsbase' + '.zip']:
    ...
    def report(duration):
        report_body = ''
        report_head = '{1:=^60}{0}{2:^60}{0}{1:=^60}{0}'.format(os.linesep, '', 'UTE Setup Report')
        ...
        for tl_name in tls_update_result:
            if tl_name in tls_update_report:
                tls_update_report[tl_name] = tls_update_report[tl_name].format(tls_update_result[tl_name])
            else:
        ...
        for result in tls_update_result.values():
            if 'PASS' == result:
                ...
                tl_fail += 1
        report_tail = report_tail.format('PASS' if tl_fail is 0 else 'FAIL', len(tls_update_result), tl_pass, tl_fail)
    ...
    def start_thread(func, *args):
        ...
        logger.info('Thread {0} calls function {1} with arguments {2}'.format(thread.name, func, args))
...
def main():
    ...
    if update_robotlte is True:
        linsee.start_thread(linsee.get_robotlte, m_isource_user, m_isource_password, 'robotlte')
    if len(m_ute_libs) is not 0:
        wheels.start_thread(wheels.get_wheels_for_libs, m_ute_libs)
    SSHService.wait_threads()
    if m_local is True:
        linsee.start_thread(linsee.ute_setup, m_ute_libs, wheels, 'FSMF', update_robotlte, m_local, m_verbose)
    else:
        for tl_id in args.tl_id:
            ip = UTE_HOST_IP[tl_id]
            ute = SSHService(ip, UTE_USER, UTE_PASSWORD, tl_id)
            ute.start_thread(ute.ute_setup, m_ute_libs, wheels, UTE_TESTLINE_TYPE[tl_id], update_robotlte, m_local,
                             m_verbose)
    SSHService.wait_threads()
