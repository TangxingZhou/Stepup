...
def args_parser(
    ...
    parser.add_argument(
        ...
        help='The number of threads for spider')
    parser.add_argument(
        '-x',
        '--xpath',
        action='store',
...
def main():
    ...
    if not args.filter == '':
        ...
        pass
    m_xpath_and_caller = {}
    if len(m_xpath_caller) is not len(
            m_xpath) or len(m_item)is not len(m_xpath):
        error_logger.error(
            'The xpaths: {} callers: {} items: {} are not corresponding'. format(
                m_xpath, m_xpath_caller, m_item))
        raise ValueError(
            'The xpaths: {} callers: {} items: {} are not corresponding'. format(
                m_xpath, m_xpath_caller, m_item))
    else:
        for index, xpath in enumerate(m_xpath):
            try:
                xpath_caller_instance = eval(
                    '{}()'.format(m_xpath_caller[index]))
                m_xpath_and_caller[xpath] = xpath_caller_instance
            except Exception as e:
                error_logger.error(
                    'Failed to create the xpath caller instance of {} for xpath: {} {}'. format(
                        m_xpath_caller[index], m_xpath, e))
