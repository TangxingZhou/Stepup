...
class DBWrapper(object):
    ...
    def __call__(self, func):
        def wrapper(*args, **kwargs):
            ...
            if 'connect' not in kwargs or not isinstance(
                    kwargs['connect'], sqlite3.Connection):
...
class SqlDB(object):
    ...
    def generate_insert_command(table_name, *items, **insert_info):
        ...
        command = (
            ...
            values)
        debug_logger.debug(
            'Generate the insert command: {} of SQLite3'.format(command))
        return command

    @DBWrapper()
    def execute(self, *command, **kwargs):
        result = True
        if 'connect' in kwargs and isinstance(
                kwargs['connect'], sqlite3.Connection):
            connect = kwargs['connect']
        else:
            connect = self.connect_db()
        debug_logger.debug(
            'Try to run the command: {} of SQLite3'.format(command))
        try:
            ...
            error_logger.error(
                'Failed to run the command: {} since {}'.format(
                    command, e))
            result = False
        return result
    ...
    def create_table(
        ...
        if item_attributes is tuple():
            ...
            if len(item_attributes) is not len(items):
                error_logger.error(
                    'In data base: {} of table: {} items: {} and attributes: {} are not corresponding' .format(
                        self._db_name, table_name, items, item_attributes))
                raise ValueError(
                    'In data base: {} of table: {} items: {} and attributes: {} are not corresponding' .format(
                        self._db_name, table_name, items, item_attributes))
            else:
        ...
        debug_logger.debug(
            'Table "{}" will be created by the command: {}'.format(
                table_name, command))
    ...
    def insert_table(self, table_name, *items, **values):
        ...
        debug_logger.debug(
            '{} will be inserted into table "{}" by the command: {}'.format(
                values, table_name, command))
    ...
    def fetchall(self, *command, **kwargs):
        ...
        try:
            ...
            error_logger.error(
                'Failed to fetchall with the command: {} since {}'.format(
                    command, e))
            result = []
        debug_logger.debug(
            'Result {} is got by the command: {}'.format(
                result, command))
        return result
...
class PageSpider(object):
    ...
    def request_page(page_url, request_header=None, data=None):
        if request_header is not None:
            if not isinstance(request_header, dict):
                error_logger.error(
                    'The request header {} must be a dict'.format(request_header))
                raise TypeError(
                    'The request header {} must be a dict'.format(request_header))
            else:
                debug_logger.debug(
                    'The request header is {}'.format(request_header))
        else:
            request_header = {}
        if data is None:
            post_data = None
            ...
            if isinstance(data, dict):
                debug_logger.debug(
                    'The request is with the data{}'.format(data))
                if data is {}:
                    post_data = None
                else:
                ...
                error_logger.error(
                    'The data {} for the request must be a dict'.format(data))
                raise TypeError(
                    'The data {} for the request must be a dict'.format(data))
        try:
            ...
            error_logger.error(
                'Failed to request the url: "{}" {}'.format(
                    page_url, e))
            return {}
        ...
        for item in response_headers:
            if len(item) is 1:
                ...
                continue
        if 'Content-Encoding' in response_header and response_header[
            ...
            info_logger.info(
                'The response header indicates that the response body is encoded by gzip')
        else:
    ...
    def spider_page(self, page_url, **kwargs):
        ...
        try:
            info_logger.info(
                'Start to request the page with the url: {}'.format(page_url))
            body = PageSpider.request_page(
                page_url, request_header=header, data=data)['body']
            info_logger.info(
                'Convert the response body to BeautifulSoup instance')
            soup = BeautifulSoup(body, 'lxml')
        except Exception as e:
            error_logger.error(
                'Failed to generate BeautifulSoup instance: {}'.format(e))
        finally:
    ...
    def parse_xpath_of_tag(self, xpath=''):
        ...
        debug_logger.debug(
            'Split "{}" into {} by "/"'.format(xpath, path_list))
        if len(path_list) < 2 or xpath.endswith('/'):
            ...
            try:
                if path_list[1:].index('') > 0:
                    error_logger.error(
                        '"{}" is not a correct xpath'.format(xpath))
                    return
                ...
                if path == '':
                    ...
                    if len(tag_info_list) is 1:
                        ...
                        for attr in tag_info_list[1:-1]:
                            attr = attr.strip()
                            debug_logger.debug(
                                'Parse the attribute: "{}" of {}'.format(
                                    attr, tag_info_list))
                            if re.match(r'\w+="[\w\s-]+"', attr) is None:
                                warn_logger.warn(
                                    '"{}" is not the correct format for attribute of a tag'.format(attr))
                                continue
                            else:
                    ...
                    debug_logger.debug(
                        'Path "{}" is parsed to {}'.format(
                            path, append_tag_info))
            debug_logger.debug(
                'Xpath "{}" is parsed to {}'.format(
                    xpath, self.xpath))
            ...
            for tag in tag_set:
                if self.specified_contents == 'text()':
                    ...
                    self.attributes[-1].append(
                        PageSpider.get_attributes(tag, *attr_select))
                    debug_logger.debug(
                        'Get the attributes: {}'.format(
                            self.attributes))
                else:
                    self.contexts[-1].append(PageSpider.get_contexts(tag))
                    debug_logger.debug(
                        'Get the contents: {}'.format(
                            self.contexts))
    ...
    def find_tag(self, tags, *tag_info):
        ...
        if isinstance(tags, element.Tag):
            ...
            error_logger.error(
                '{} is not an instance of element.Tag or element.ResultSet'.format(
                    type(tags)))
            raise TypeError(
                '{} is not an instance of element.Tag or element.ResultSet'.format(
                    type(tags)))
        if len(tag_info) is 0:
            error_logger.error(
                '{} has no information of the tag'.format(tag_info))
            raise ValueError(
                '{} has no information of the tag'.format(tag_info))
        else:
            pass
        for tag in tags:
            if self._flag_all is False and len(
                    self.tags) is one_element_length:
            ...
            if isinstance(tag, element.Tag):
                ...
                if isinstance(
                    info,
                    dict) and len(info) is one_element_length and isinstance(
                    tuple(
                        info.values())[first_elem_index],
                        dict):
                    ...
                    debug_logger.debug(
                        'Tag name is "{}", attributes are {}'.format(
                ...
                error_logger.error(
                    '{} is not an instance of element.Tag'.format(
                        type(tag)))
                raise TypeError(
                    '{} is not an instance of element.Tag'.format(
                        type(tag)))
    ...
    def get_attributes(tag, *attr_key):
        ...
        try:
            ...
            error_logger.error(
                'Failed to get the attributes of tag "{}" : {}'.format(
                    name, e))
            return {}
        attributes_select = {}
        for key in attr_key:
            if key in attributes:
                ...
                warn_logger.warn(
                    '"{}" is not the attribute of tag "{}"'.format(
                        key, name))
        if len(attr_key) is 0:
            debug_logger.debug(
                'The attributes of tag "{}" are {}'.format(
                    name, attributes))
            return attributes
        else:
            debug_logger.debug(
                'The specified attributes of tag "{}" are {} with the names {}'. format(
                    name, attributes, attr_key))
    ...
    def get_text(tag):
        ...
        try:
            ...
            error_logger.error(
                'Failed to get the text of "{}" : {}'.format(
                    name, e))
        finally:
    ...
    def get_contexts(tag):
        ...
        try:
            ...
            error_logger.error(
                'Failed to get the contexts of "{}" : {}'.format(
                    name, e))
        finally:
            debug_logger.debug(
                'The contexts of tag "{}" is {}'.format(
                    name, contexts))
        return contexts
    ...
    def retrieve(url, filename=None, **kwargs):
        if os.path.exists(filename):
            ...
            if os.path.exists(filename.rpartition(os.sep)[0]):
                ...
                info_logger.info(
                    'Directory "{}" is made'.format(
                        filename.rpartition(
                            os.sep)[0]))
            try:
                local_file_name = request.urlretrieve(url, filename, **kwargs)
                info_logger.info(
                    '"{}" is retrieved from "{}"'.format(
                        local_file_name, url))
            except Exception as e:
                error_logger.error(
                    'Failed to get "{}" from "{}" since: {}'.format(
                        filename, url, e))
...
class WebSpider(object):
    ...
    def get_into_new_page(self, **kwargs):
        while True:
            try:
                ...
                debug_logger.debug(
                    'Get into the new page with the url: {}'.format(new_url))
                return self.get_elements_from_page(
                    new_url, self._web_caller, **kwargs)
    ...
    def get_elements_from_page(self, new_url, caller_in_page=None, **kwargs):
        if 'page_spider' in kwargs and isinstance(
                kwargs['page_spider'], PageSpider):
            page_spider = kwargs['page_spider']
        else:
            error_logger.error(
                'PageSpider instance is not correctly specified')
        ...
        try:
            ...
            for xpath, caller in self._xpath_and_caller.items():
                ...
                info_logger.info(
                    'The instance of {} starts to process the contents specified by the xpath: "{}"'. format(
                        caller.__class__, xpath))
                caller(page_spider)
                temp_inst = copy.copy(page_spider)
                xpath_result_in_page.append(vars(temp_inst))
            info_logger.info(
                'The instance of {} starts to process the contents found in this page'. format(
                    caller_in_page.__class__))
    ...
    def start_threads(self):
        ...
        if not isinstance(
            ...
            error_logger.error(
                'The caller: {} for web is not correctly specified'.format(
                    self._web_caller))
            raise ValueError(
                'The caller: {} for web is not correctly specified'.format(
                    self._web_caller))
        if isinstance(self._xpath_and_caller, dict):
            for xpath, caller in self._xpath_and_caller.items():
                if isinstance(
                        xpath, str) and not isinstance(
                        caller, type) and callable(caller):
                    continue
                else:
                    error_logger.error(
                        'The caller: {} to process the xpath: {} are not correctly specified'. format(
                            caller, xpath))
                    raise ValueError(
                        'The caller: {} to process the xpath: {} are not correctly specified'. format(
                            caller, xpath))
        ...
        for i in range(self._threads_number):
            ...
            thread_request_pages = threading.Thread(
                target=self.get_into_new_page, kwargs=kw)
            thread_request_pages.start()
            info_logger.info(
                'Thread {} starts'.format(
                    thread_request_pages.name))
            info_logger.info(
                'Thread {} calls function {} with arguments {}'. format(
                    thread_request_pages.name, self.get_into_new_page, kw))
